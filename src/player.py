        ...
        for unite in self.game_map.listeUnites:
            ...
            if unite.identifiant == "V" and unite.appartenance == 1:
                ...
                if unite.role == 1:
                    ...
                    moves = src.util.nextPositions((unite.position[0], unite.position[1]), self.game_map, (unite.target[0], unite.target[1], unite.pointMouvement))
                    '''
                ...
                if unite.role == 0:
                    ...
                    posLibreRessource = src.util.closestAvailableRessource(unite, self.game_map)
                    unite.target = posLibreRessource
                    # on save que cette position est prise
                    self.game_map.target[posLibreRessource[0]][posLibreRessource[1]]=True
                    # on deplace
                    deplacementUnite = src.util.nextPositions(unite.position,self.game_map,unite.target,unite.pointMouvement)
